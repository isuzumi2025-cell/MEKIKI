"""
統合テキスト抽出モジュール (スタンドアロン版)
Web DOM, PDF OCR, 画像OCRからテキストを抽出

内蔵エンジン:
- engines/dom_handler.py: DOM/XPath テキスト抽出
- engines/ocr_engine.py: Google Vision OCR
- engines/capture.py: Playwright Screenshot
"""
import os
import io
import json
...
class TextExtractor:
    ...
    def __init__(self, credentials_path: Optional[str] = None):
        ...
        """
        # 認証情報のデフォルトパス
        default_creds = Path(__file__).parent.parent.parent / "credentials" / "service_account.json"
        self.credentials_path = credentials_path or str(default_creds)
    ...
    def extract_from_web(
        ...
        """
        try:
            from app.core.engines.dom_handler import DomXPathHandler
        except ImportError:
    ...
    def extract_from_image(
        ...
        Returns:
            {
                ...
                "path": str,
                "full_text": str,
                "blocks": [{"text": str, "bbox": [...], "confidence": float, "type": str}, ...]
            }
        """
        result = {
            "source": "ocr",
        ...
        if not self._ocr_engine:
            try:
                from app.core.engines.ocr_engine import OCREngine
...
def quick_extract_web(url: str, **kwargs) -> Dict:
    ...
    return TextExtractor().extract_from_web(url, **kwargs)


def quick_extract_pdf(pdf_path: str, **kwargs) -> Dict:
    """PDFからテキストをクイックExtraction"""
    return TextExtractor().extract_from_pdf(pdf_path, **kwargs)


def quick_extract_image(image_path: str, **kwargs) -> Dict:
    """画像からテキストをクイックExtraction"""
    return TextExtractor().extract_from_image(image_path, **kwargs)
