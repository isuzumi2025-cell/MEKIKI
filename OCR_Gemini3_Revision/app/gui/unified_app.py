...
class UnifiedApp(ctk.CTk):
    ...
    def _start_new_crawl(self):
        ...
        ctk.CTkButton(profile_frame, text="💾", width=30, command=save_profile, fg_color="#4CAF50").pack(side="left", padx=2)
        ctk.CTkButton(profile_frame, text="🗑️", width=30, command=delete_profile, fg_color="#F44336").pack(side="left", padx=2)
        ...
        pass_toggle_btn = ctk.CTkButton(pass_input_frame, text="👁️", width=35, command=toggle_password, fg_color="#616161")
        ...
        def run_crawl():
            ...
            def crawl_thread():
                try:
                    ...
                    self.after(0, lambda: messagebox.showerror("エラー", f"クロールエラー: {e}"))
                    self.after(0, lambda: progress_label.configure(text=f"❌ エラー: {e}"))
